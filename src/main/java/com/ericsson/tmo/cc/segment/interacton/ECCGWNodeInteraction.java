...
public class ECCGWNodeInteraction extends AbstractNodeInteraction {
	...
	private static String execute(String request,  ConfigPropertiesPojo configPropertiesPojo) {
		...
		try {
			HttpPost httpPost = new HttpPost(configPropertiesPojo.getEccUrl());
			String requestPayload = request.replace(SegmentConstants.TRANS_ID_DATA,
					GenericUtils.fetchOriginTransactionId());
			System.out.println("ECC Gw request: " + requestPayload);
			
			String hexString = GenericUtils.toShaHexString(configPropertiesPojo.getEccPassword() + GenericUtils.getTimestamp());
			...
			params.add(new BasicNameValuePair(SegmentConstants.PARTNERID, SegmentConstants.THIRTY_NINE));
